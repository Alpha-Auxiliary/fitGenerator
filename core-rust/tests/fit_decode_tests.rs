use fit_generator_core::{
    ...
    build_activity_model, generate_fit, parse_and_validate,
};
use fitparser::{from_bytes, profile::MesgNum, FitDataRecord};
...
fn assert_field_names(record: &FitDataRecord, expected: &[&str]) {
    let mut actual = record.fields().iter().map(|field| field.name().to_owned()).collect::<Vec<_>>();
    actual.sort();
    let mut expected = expected.iter().map(|name| (*name).to_owned()).collect::<Vec<_>>();
    expected.sort();
    assert_eq!(actual, expected, "unexpected fields for {:?}", record.kind());
}

fn field_display(record: &FitDataRecord, name: &str) -> String {
    let field = record.fields().iter().find(|field| field.name() == name)
        .unwrap_or_else(|| panic!("missing {name} on {:?}", record.kind()));
    format!("{}", field.value())
}

fn normalized_field_display(record: &FitDataRecord, name: &str) -> String {
    field_display(record, name).trim_matches('"').to_ascii_lowercase()
}
...
fn assert_scaled_value(record: &FitDataRecord, name: &str, raw: u64, scale: f64) {
    ...
    assert!((actual - expected).abs() <= 0.000_1, "{name}: {actual} != {expected}");
}
...
fn expected_average_heart_rate(samples: &[ActivitySample]) -> i64 {
    let total = samples.iter().map(|sample| i64::from(sample.heart_rate_bpm)).sum::<i64>();
...
fn generated_fit_has_a_valid_header_crc_and_required_messages() {
    ...
    assert_eq!(usize::try_from(u32::from_le_bytes(bytes[4..8].try_into().unwrap())).unwrap(), bytes.len() - 16);
    assert_eq!(u16::from_le_bytes(bytes[12..14].try_into().unwrap()), fit_crc(&bytes[..12]));
    assert_eq!(u16::from_le_bytes(bytes[bytes.len() - 2..].try_into().unwrap()), fit_crc(&bytes[..bytes.len() - 2]));
    ...
    for kind in [
        ...
        assert!(records.iter().any(|record| record.kind() == kind), "missing {kind:?}");
    }
}
...
fn decoded_messages_have_canonical_data_order_counts_and_fields() {
    ...
    assert_eq!(records_by_kind(&records, MesgNum::Record).len(), model.samples.len());
    assert_eq!(records_by_kind(&records, MesgNum::Lap).len(), model.laps.len());

    let file = records_by_kind(&records, MesgNum::FileId);
    assert_eq!(file.len(), 1);
    assert_field_names(file[0], &["type", "manufacturer", "product", "serial_number", "time_created"]);
    ...
    assert_field_names(device[0], &["timestamp", "device_index", "manufacturer", "product"]);
    ...
    for record in &decoded_records {
        assert_field_names(record, &["timestamp", "position_lat", "position_long", "heart_rate", "distance", "speed"]);
    }
    for (record, sample) in [
        (decoded_records[0], &model.samples[0]),
        (decoded_records[decoded_records.len() - 1], model.samples.last().unwrap()),
    ] {
        assert_eq!(field_i64(record, "position_lat"), i64::from(sample.position_lat_semicircles));
        assert_eq!(field_i64(record, "position_long"), i64::from(sample.position_long_semicircles));
        assert_eq!(field_i64(record, "heart_rate"), i64::from(sample.heart_rate_bpm));
    ...
    for (record, lap) in laps.into_iter().zip(&model.laps) {
        assert_field_names(record, &[
            "timestamp", "event", "event_type", "start_time", "start_position_lat",
            "start_position_long", "end_position_lat", "end_position_long", "total_elapsed_time",
            "total_timer_time", "total_distance", "avg_speed", "max_speed", "avg_heart_rate",
            "max_heart_rate",
        ]);
        ...
        assert_scaled_value(record, "avg_speed", expected_average_speed(lap.distance_cm, lap.duration_ms), 1_000.0);
        assert_scaled_value(record, "max_speed", u64::from(
            model.samples[lap.start_sample..=lap.end_sample].iter().map(|sample| sample.speed_mm_per_sec).max().unwrap(),
        ), 1_000.0);
        assert_eq!(field_i64(record, "avg_heart_rate"), expected_average_heart_rate(&model.samples[lap.start_sample..=lap.end_sample]));
        assert_eq!(field_i64(record, "max_heart_rate"), i64::from(
            model.samples[lap.start_sample..=lap.end_sample].iter().map(|sample| sample.heart_rate_bpm).max().unwrap(),
        ));
    }

    let session = records_by_kind(&records, MesgNum::Session);
    assert_eq!(session.len(), 1);
    assert_field_names(session[0], &[
        "timestamp", "event", "event_type", "start_time", "start_position_lat", "start_position_long",
        "sport", "sub_sport", "total_elapsed_time", "total_timer_time", "total_distance", "avg_speed",
        "max_speed", "avg_heart_rate", "max_heart_rate", "first_lap_index", "num_laps",
    ]);
    ...
    assert_scaled_value(session[0], "total_elapsed_time", model.total_duration_ms, 1_000.0);
    assert_scaled_value(session[0], "total_timer_time", model.total_duration_ms, 1_000.0);
    assert_scaled_value(session[0], "total_distance", model.total_distance_cm, 100.0);
    assert_scaled_value(session[0], "avg_speed", expected_average_speed(model.total_distance_cm, model.total_duration_ms), 1_000.0);
    assert_scaled_value(session[0], "max_speed", u64::from(model.samples.iter().map(|sample| sample.speed_mm_per_sec).max().unwrap()), 1_000.0);
    assert_eq!(field_i64(session[0], "avg_heart_rate"), expected_average_heart_rate(&model.samples));
    assert_eq!(field_i64(session[0], "max_heart_rate"), i64::from(model.samples.iter().map(|sample| sample.heart_rate_bpm).max().unwrap()));
    assert_eq!(field_i64(session[0], "num_laps"), i64::try_from(model.laps.len()).unwrap());

    let activity = records_by_kind(&records, MesgNum::Activity);
    assert_eq!(activity.len(), 1);
    assert_field_names(activity[0], &["timestamp", "total_timer_time", "num_sessions", "type", "event", "event_type"]);
    ...
    assert_scaled_value(activity[0], "total_timer_time", model.total_duration_ms, 1_000.0);
